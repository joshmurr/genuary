...
void main(){
    if(i_Age >= i_Life) {
        ...
        float x = sin(theta)*cos(phi) * 0.5;
        float y = sin(theta)*sin(phi) * 0.5;
        float z = cos(theta) * 0.5;
...
// 
        v_Position = rotateY(i_Position, u_TotalTime * 0.001) + i_Velocity * u_TimeDelta;
        ...
        v_Velocity = i_Velocity + u_Gravity * u_TimeDelta + force * u_Force * u_TimeDelta;
    }
}
